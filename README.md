...
## Instructions
Theres a `XML-DTD_Parser.jar` file that you should install into your local maven repository; this file is used to parse XML DTD files that are used in the project. The file is located in the root of the project and was created apart this project to avoid any issues with the project itself.
Here you can find the project that generates the `XML-DTD_Parser.jar` file: [XML-DTD_Parser](https://github.com/JNArrazola/XML-DTD_Parser) 

> **Advise**: The `XML-DTD_Parser` project is in spanish. 

To do so, you should run the following command:

```bash
mvn install:install-file -Dfile=XML-DTD_Parser.jar -DgroupId=org.iesfm -DartifactId=XML-DTD_Parser -Dversion=1.0 -Dpackaging=jar
```
